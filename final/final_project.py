...
def preprocess():
    ...
    for var in categorical:
        lbl = LabelEncoder()
        # We also include the test values, even if they do not influence the training. 
		# You could simply set the values, which only in the test occur to -1, but this 
		# is easier this way
    ...
    # Count Vectorizer to the features = Create features that specify how often every word 
	# of all words of the corpus of the features in each listing occur. For this we need to 
	# combine the list of features into a string, so first connect the related feature 
	# concepts to _ and then the list Of the features to make a string
    ...
    # Sparse and dense data, tocsr = to compressed sparse row format
